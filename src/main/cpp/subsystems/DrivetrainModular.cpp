...
DifferentialDrive = new frc::DifferentialDrive(motor_R_main, motor_L_main);
...
void DrivetrainModular::StandardDrive(frc::Joystick* driveStick) {
  ...
  //  frc::SmartDashboard::PutNumber("MaxSpeed", (((driveStick->GetRawAxis(3)*-1)+1)*0.5));
    DifferentialDrive->SetMaxOutput(((driveStick->GetRawAxis(3)*-1)+1)*0.5);
    DifferentialDrive->ArcadeDrive(driveStick->GetX()/*maxSpeed*/, driveStick->GetY()*-1/**maxSpeed*/, false);
}
