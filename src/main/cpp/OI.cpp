...
frc2::JoystickButton GrabberExtendSideTrigger(driveStick, 2);
//frc2::JoystickButton GrabberExtendSideTrigger1(driveStick, 1);
...
TelescopeExtendButton.WhileTrue(TelescopeExtend(&pneumaticarm).ToPtr());
TelescopeRetractButton.WhileTrue(TelescopeRetract(&pneumaticarm).ToPtr());
...
ElevatorUpButton.WhileTrue(ElevatorUp(&m_elevator, &pneumaticarm, false).ToPtr());
ElevatorDownButton.WhileTrue(ElevatorDown(&m_elevator, &pneumaticarm, false).ToPtr());
GrabberFrontTrigger.WhileTrue(GrabberGrab(&pneumaticgrabber).ToPtr());
GrabberExtendSideTrigger.ToggleOnTrue(frc2::cmd::StartEnd([&] { pneumaticextender.ExtendGrabber(); },
    [&] { pneumaticextender.RetractGrabber(); },
    {&pneumaticextender}));
...
frc2::Trigger(isDown).WhileTrue(ElevatorUp(&m_elevator, &pneumaticarm, false).ToPtr());
frc2::Trigger(isDown).WhileTrue(TelescopeRetract(&pneumaticarm).ToPtr());
frc2::Trigger(isUp).WhileTrue(ElevatorDown(&m_elevator, &pneumaticarm, true).ToPtr());
frc2::Trigger(isUp).WhileTrue(TelescopeExtend(&pneumaticarm).ToPtr());
